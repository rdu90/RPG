...
The codebase has completed milestones **M0–M4**: engine/transport/persistence skeleton, a bubbletea shell, a seeded galaxy with fly/turn-spend movement, haggling-based trade (multi-round negotiation replacing static prices, plus per-system reputation and derived alignment), exploration (scouting adjacent systems at half the flight turn cost, fog of war, and discoverable per-system anomaly secrets), and expansion (founding a colony at the player's current system with a chosen production-focus commodity; population grows toward a development-level-derived cap on a coarse galaxy tick computed lazily from elapsed real time, and each tick also decays the Focus commodity's local market price toward a floor — the mechanism by which colony production feeds back into markets) are all implemented and playable. `internal/engine/{galaxy,economy,haggle,explore,colony,turn,player}` are populated; `fleet`, `techtree`, `espionage`, `combat` are still empty packages reserved for later milestones (M5+). `cmd/rpg-server` is likewise an empty placeholder for M9 (async multiplayer).
