...
The codebase has completed milestones **M0–M3**: engine/transport/persistence skeleton, a bubbletea shell, a seeded galaxy with fly/turn-spend movement, haggling-based trade (multi-round negotiation replacing static prices, plus per-system reputation and derived alignment), and exploration (scouting adjacent systems at half the flight turn cost, fog of war, and discoverable per-system anomaly secrets) are all implemented and playable. `internal/engine/{galaxy,economy,haggle,explore,turn,player}` are populated; `fleet`, `colony`, `techtree`, `espionage`, `combat` are still empty packages reserved for later milestones (M4+). `cmd/rpg-server` is likewise an empty placeholder for M9 (async multiplayer).
