...
The codebase has completed milestones **M0–M8**: engine/transport/persistence skeleton, a bubbletea shell, a seeded galaxy with fly/turn-spend movement, haggling-based trade (multi-round negotiation replacing static prices, plus per-system reputation and derived alignment), exploration (scouting adjacent systems at half the flight turn cost, fog of war, and discoverable per-system anomaly secrets), expansion (founding a colony at the player's current system with a chosen production-focus commodity; population grows toward a development-level-derived cap on a coarse galaxy tick computed lazily from elapsed real time, and each tick also decays the Focus commodity's local market price toward a floor — the mechanism by which colony production feeds back into markets), a tech tree (a small data-driven catalog — four branches of six tiers each, gated by a linear per-branch prerequisite chain — researched one project at a time, accruing points toward the active tech on the same lazy, elapsed-time tick model as colony growth; completed techs grant a one-time permanent effect: more cargo capacity, a higher turn allowance, reduced NPC greed in future haggling, or a faster future research rate), espionage (recruit spies for a flat credit/turn cost, then send one on a narrow set of missions — steal, sabotage, intel — against any galaxy system regardless of distance or discovery; each mission resolves as a single deterministic probability check, spy Skill against a target's development-level-derived CounterIntel, with no detection minigame or persistent spy-network simulation; a failed mission has a further chance of losing the spy to capture), fleet combat (arriving at a system via `Move` rolls a chance of a hostile encounter, weighted toward lower-development, less-patrolled frontier systems; an encountered `combat.Hostile` is a plain value round-tripped by the caller — the same session-passing shape as `haggle.Session` — into a `ResolveEncounter` command, which either resolves an `AttemptFlee` probability check or runs `combat.Fight`, a fully deterministic, non-random formula over discrete rounds capped at 10, producing a battle log; victory grants a credit reward scaled to the hostile's toughness, defeat costs a fraction of credits and cargo and drops the ship's hull to 0, and a `RepairShip` command restores hull for a flat per-point credit cost — there is no permadeath), and planetary bombardment/invasion (a handful of the galaxy's systems are seeded at creation with a rival-faction-owned colony — a small data-driven `faction` catalog, chosen from systems at or above a development-level threshold — each carrying a persisted defensive `Garrison` (`fleet.Stats`); `Bombard` is a risk-free orbital strike that wears the garrison down to a floor and chips population, both acting on whatever colony exists at the player's current system, mirroring `Colonize`'s implicit-current-system targeting; `Invade` wraps the garrison in a synthetic `combat.Hostile` and resolves it through the same unmodified `combat.Fight` used by M7 hostiles — victory flips the colony's `Owner` to the player (its population/production carry over unchanged into the existing tick/decay mechanism) while defeat costs credits/cargo exactly like a lost `ResolveEncounter` and still wears the garrison down, so a failed invasion is never wasted) are all implemented and playable. `internal/engine/{galaxy,economy,haggle,explore,colony,techtree,espionage,fleet,combat,faction,turn,player}` are populated. `cmd/rpg-server` remains an empty placeholder for M9 (async multiplayer), the only milestone left before M10 (platform extraction).
...
- **`internal/tui`** — bubbletea (Elm-architecture Init/Update/View) + lipgloss + bubbles. `app/` is the root model/program; `screens/` and `components/` (currently empty, reserved for upcoming milestones) will hold per-feature screens and shared widgets; `style/` holds the shared theme; `companion/` holds the game's single narrative voice (a Jeeves-like tone) as plain string-returning functions, the retrofit point for later alignment-driven tone variants. The TUI only ever talks through `internal/transport`, never reaches into `internal/engine` directly.
